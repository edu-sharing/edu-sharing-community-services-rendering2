...
class H5pUploadService(
    ...
    private fun getLumiId(cacheObject: CacheObject): String {
        try {
            ...
            trackingService.trackCacheObject(cacheObject, lumiContentManagementService.getContentBucket(repoId))
            return lumiId
        ...
        val size = storageService.getDirectorySize(lumiContentManagementService.getContentBucket(repoId), lumiId)
        trackingService.trackCacheObject(cacheObject, lumiContentManagementService.getContentBucket(repoId), size)
        return lumiId
    }
