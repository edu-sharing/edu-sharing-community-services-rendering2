 ...
 * [ImportQueueProperties], [SingleActiveQueueProperties]) rather than read from `app.queue.<x>.mode`:
 *
...
abstract class QueueSpec {
    ...
    /** Fixed by the concrete subclass ([StandardQueueProperties]/[ImportQueueProperties]/[SingleActiveQueueProperties]). */
...
/**
 * Base for a decoupled high-fan-out import queue (one channel per pod; K = [concurrency] sizes the dedicated
 * import HTTP pool + broker prefetch). Fails fast if a configured prefetch would leave HTTP connections idle.
 */
abstract class ImportQueueProperties : QueueSpec() {
    override val mode = QueueMode.IMPORT

    @PostConstruct
    fun validateImportPrefetch() {
        val prefetch = prefetch
        require(prefetch == null || prefetch >= concurrency) {
            "Import queue prefetch ($prefetch) must be >= concurrency ($concurrency), or HTTP connections sit idle."
        }
    }
}
